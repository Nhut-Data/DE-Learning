...
INSERT INTO `customers` VALUES (11,'Levy','Mynett','1969-10-13','404-246-3370','68 Lawn Avenue','Atlanta','GA',7961);
...
INSERT INTO `orders` VALUES (7,2,'2018-09-22',3,NULL,'2018-09-23',4);
...
INSERT INTO `orders` VALUES (10,6,'2018-04-22',1,NULL,'2018-04-23',2);
INSERT INTO `orders` VALUES (11,6,'2018-04-22',3,NULL,'2018-04-23',2);
INSERT INTO `orders` VALUES (12,11,'2019-04-22',3,NULL,'2018-04-23',2);
...
-- Bài 1. Lấy danh sách khách hàng ở tiểu bang VA hoặc FL, chỉ hiển thị first_name, last_name, 
SELECT first_name, last_name, state
FROM customers
WHERE state = 'VA' OR state = 'FL';
-- Bài 2. Lấy tất cả sản phẩm có unit_price >= 2.00, sắp xếp giá từ cao xuống thấp
SELECT *
FROM products
WHERE unit_price >= 2.00 
ORDER BY unit_price DESC;
-- Bài 3. Tìm nhân viên có lương trên 90,000 — hiển thị họ tên và lương
SELECT first_name, last_name, salary
FROM employees
WHERE salary > 90000;
-- Bài 4. Lấy danh sách invoices chưa thanh toán (payment_date là NULL), hiển thị invoice_id, number, invoice_total
SELECT invoice_id, number, invoice_total 
FROM invoices
WHERE payment_date IS NULL;
-- Bài 5. Lấy danh sách orders kèm first_name, last_name của khách hàng và tên shipper — nếu chưa có shipper thì hiện NULL
SELECT o.*, c.first_name, c.last_name, s.name
FROM orders o 
LEFT JOIN customers c ON c.customer_id = o.customer_id
LEFT JOIN shippers s ON s.shipper_id = o.shipper_id;
-- Bài 6. Tính tổng tiền mỗi khách hàng đã mua (dựa vào order_items), hiển thị customer_id và total_spent, sắp xếp từ cao xuống
SELECT o.customer_id, 
SUM(oi.quantity + oi.unit_price) AS total_spent
FROM orders o
JOIN order_items oi ON oi.order_id = o.customer_id
GROUP BY o.customer_id
ORDER BY total_spent DESC;
-- Bài 7. Lấy tên nhân viên kèm tên thành phố văn phòng họ đang làm việc
SELECT e.first_name, o.city, o.state
FROM employees e
JOIN offices o ON o.office_id = e.office_id;
-- Bài 8. Với mỗi client trong sql_invoicing, đếm số invoice và tính tổng invoice_total
SELECT client_id,  
COUNT(client_id) AS count_client,
SUM(client_id) AS invoice_total
FROM invoices 
GROUP BY client_id;
-- Bài 9. Tìm khách hàng chưa từng đặt hàng lần nào — dùng subquery hoặc LEFT JOIN đều được
SELECT c.last_name, c.customer_id AS khach_hang, 
o.customer_id AS ko_co_don_hang
FROM customers c
...
-- Bài 10. Lấy danh sách nhân viên kèm tên manager của họ — ai không có manager thì hiện NULL ở cột manager (CÂU KHÔNG TỰ LÀM ĐƯỢC VÌ QUÁ ĐỘC LẠ)
SELECT e.last_name AS nhan_vien,
m.last_name AS manager 
FROM employees e
LEFT JOIN employees m ON e.reports_to = m.reports_to; 
-- Bài 11. Tìm các invoices có invoice_total cao hơn mức trung bình của tất cả invoices
SELECT invoice_id ,invoice_total
FROM invoices
WHERE invoice_total > (SELECT AVG(invoice_total)
FROM invoices);
-- Bài 12. Với mỗi client, lấy invoice có invoice_date mới nhất
SELECT i.invoice_id, i.invoice_date
FROM invoices i
JOIN clients c ON c.client_id = i.client_id
WHERE i.invoice_date = (SELECT MAX(invoice_date) FROM invoices);
-- cách giải đúng 
SELECT client_id, MAX(invoice_date) AS neww_date
FROM invoices
GROUP BY client_id;
-- Bài 13. Xếp hạng nhân viên theo lương trong từng văn phòng bằng RANK() — ai lương cao nhất thì rank = 1
SELECT employee_id,
RANK() OVER (PARTITION BY office_id ORDER BY salary DESC) AS xep_hang_luong
FROM employees;
-- Bài 14. Tính running total (cộng dồn) số tiền thanh toán theo ngày trong sql_invoicing, sắp xếp theo ngày tăng dần
SELECT payment_id, 
date,
amount,
SUM(amount) OVER ( ORDER BY date ASC) AS running_total 
FROM payments;
-- Bài 15. Phân loại khách hàng theo cột points: trên 3000 = VIP, từ 1000 đến 3000 = Regular, dưới 1000 = New — dùng CASE_WHEN
SELECT customer_id, last_name, points, 
CASE
WHEN points > 3000 THEN 'VIP'
WHEN points >= 1000 AND points <= 3000 THEN 'Regular'
ELSE 'New'
END AS phan_loai
FROM customers; 

-- Viết các query tạo báo cáo tổng hợp từ sql_store:
-- Top 3 khách hàng chi tiêu nhiều nhất
SELECT customer_id, last_name, points
FROM customers
ORDER BY points DESC LIMIT 3;
-- Sản phẩm bán chạy nhất theo tổng số lượng
SELECT p.name,
...
ORDER BY doanh_thu DESC LIMIT 1;
-- Tỉ lệ đơn hàng theo từng trạng thái (Processed / Shipped / Delivered)
SELECT os.name,
COUNT(*) AS so_don,
ROUND(COUNT(*) / (SELECT COUNT(*) FROM orders)*100 , 1) AS ti_le
FROM orders o
JOIN order_statuses os ON os.order_status_id = o.status
GROUP BY os.name;
-- Khách hàng nào thuộc loại VIP nhưng chưa có đơn hàng nào năm 2018?
SELECT customer_id,
points  AS VIP
FROM customers 
WHERE points >3000 
AND customer_id NOT IN  (SELECT customer_id FROM orders WHERE YEAR(order_date) = 2018);
